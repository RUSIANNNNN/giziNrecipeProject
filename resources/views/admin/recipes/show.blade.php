    ...
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        <div class="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
            <div class="grid grid-cols-1 md:grid-cols-5">
                <div class="md:col-span-2">
                    ...
                    @else
                        <div
                            class="h-56 md:h-full w-full flex items-center justify-center bg-slate-100 text-sm text-slate-500">
                            Tidak ada foto
                        </div>
                    @endif
                </div>
                <div class="md:col-span-3 p-5 space-y-3">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-slate-700">
                        <div class="space-y-1">
                            <p class="text-xs font-semibold text-slate-500">Pakar Gizi</p>
                            <p>{{ $recipe->nutritionist ?? '-' }}</p>
                        </div>
                        <div class="space-y-1">
                            <p class="text-xs font-semibold text-slate-500">Durasi</p>
                            <p>{{ $recipe->duration ?? '-' }}</p>
                        </div>
                        <div class="space-y-1 sm:col-span-2">
                            <p class="text-xs font-semibold text-slate-500">Deskripsi</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div class="lg:col-span-2 space-y-5">
                <div class="bg-white border border-slate-100 rounded-2xl shadow-sm p-5">
                    <h2 class="text-sm font-semibold text-slate-900 mb-3">Bahan</h2>
                    <ul class="list-disc list-inside text-sm text-slate-700 space-y-1.5">
                        @forelse ($recipe->ingredients as $ingredient)
                            <li>{{ $ingredient->item }}</li>
                        @empty
                            <li class="text-slate-400 text-sm"><em>Tidak ada data bahan.</em></li>
                        @endforelse
                    </ul>
                </div>

                <div class="bg-white border border-slate-100 rounded-2xl shadow-sm p-5">
                    <h2 class="text-sm font-semibold text-slate-900 mb-3">Langkah</h2>
                    <ol class="list-decimal list-inside text-sm text-slate-700 space-y-1.5">
                        @forelse ($recipe->steps as $step)
                            <li>{{ $step->instruction }}</li>
                        @empty
                            <li class="text-slate-400 text-sm"><em>Tidak ada langkah tersedia.</em></li>
                        @endforelse
                    </ol>
                </div>
            </div>
            ...
            <div class="space-y-5">
                <div class="bg-white border border-slate-100 rounded-2xl shadow-sm p-5">
                    <h2 class="text-sm font-semibold text-slate-900 mb-3">Kandungan Gizi per Porsi</h2>
                    <ul class="text-sm text-slate-700 space-y-1.5">
                        @forelse ($recipe->nutritions as $nutrition)
                            <li class="flex items-center justify-between gap-2">
                                <span>{{ $nutrition->name }}</span>
                                <span class="text-slate-900 font-medium">{{ $nutrition->value }}</span>
                            </li>
                        @empty
                            <li class="text-slate-400 text-sm"><em>Tidak ada data kandungan gizi.</em></li>
                        @endforelse
                    </ul>
                </div>
        ...
        <div class="bg-white border border-slate-100 rounded-2xl shadow-sm p-5 space-y-4">
            <div class="flex items-center justify-between">
                <h2 class="text-sm font-semibold text-slate-900">Komentar Pengguna</h2>
                            ...
                            <div class="flex-1">
                                <div class="flex items-center justify-between gap-2">
                                    ...
                                    <form
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="inline-flex items-center rounded-lg border border-rose-400 px-2.5 py-1 text-[11px] font-medium text-rose-600 hover:bg-rose-50">
                                            Hapus
                                        </button>
                                    </form>
                                </div>
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
                <a href="{{ route('admin.recipes.edit', $recipe->id) }}"
                    class="inline-flex items-center rounded-lg border border-emerald-500 px-4 py-2 text-xs md:text-sm font-medium text-emerald-700 hover:bg-emerald-50">
                    Edit Resep
                </a>
            </div>
        </div>
